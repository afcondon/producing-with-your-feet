...
handleAction = case _ of
  ...
  -- |
  -- | Sends a bank-names request, then a switch-names request per bank. Replies
  -- | arrive asynchronously on `mc6Input` and are handled in `MC6MidiReceived`;
  -- | nothing here waits for them, because a read that hangs waiting for a
  -- | device that did not answer is worse than one that simply reports nothing
  -- | came back.
  -- |
  -- | Deliberately outside `withEditorSession`: that wrapper opens an *upload*
  -- | session, and a read should never be able to leave the device in a state
  -- | where a half-finished write could land.
  ReadMC6Banks -> do
    st <- H.get
    case st.connections.mc6Output of
      Nothing ->
        H.modify_ _ { midiTest = Just "Cannot read: no MC6 SysEx output selected." }
      Just output -> do
        H.modify_ _ { mc6ReadStatus = Just "Reading bank names\x2026", mc6BankNames = Map.empty, mc6BankSwitches = Map.empty }
        sendSysExLogged "read-bank-names" output Read.requestBankNames
        H.liftAff (delay (Milliseconds 250.0))
        for_ (Array.range 0 (Survey.bankCount - 1)) \b -> do
          sendSysExLogged ("read-bank-" <> show b) output (Read.requestBankSwitches b)
          H.liftAff (delay (Milliseconds 80.0))
        H.modify_ _ { mc6ReadStatus = Just "Requests sent \x2014 waiting for replies." }
